...
### Code is as pretty as Drexl, needs a tidy and de-cut-and-paste
...
for (c_level in c(0.60, 0.80, 0.90, 0.95, 0.98, 0.99)) {
  ...
  ### Some guesswork going on here
  u_model1 <- lm(est_afr_upper ~ poly(failures, 1), data=data)
  u_model2 <- lm(est_afr_upper ~ poly(failures, 2), data=data)
  u_model3 <- nls(est_afr_upper~b / failures + a, data=data,
                  start=list(a=1, b=1))
  u_model4 <- nls(est_afr_upper~b * log(failures) + a, data=data,
                  start=list(a=1, b=1))
  u_model5 <- nls(est_afr_upper~1 / failures * b * log(failures) + a, data=data,
                  start=list(a=1, b=1))
  u_model6 <- nls(est_afr_upper~b / failures * log(failures + c) + a, data=data,
                  start=list(a=1, b=1, c=0))
  u_model7 <- nls(est_afr_upper~b * failures^d * log(failures + c) + a, data=data,
                  start=list(a=1, b=1, c=0, d=-1))
  
  l_model6 <- nls(est_afr_lower~b / failures * log(failures + c) + a, data=data,
                  start=list(a=1, b=1, c=0))
  ### d=-0.8 rather than -1 to avoid
  ### In log(failures + c) : NaNs produced
  l_model7 <- nls(est_afr_lower~b * failures^d * log(failures + c) + a, data=data,
                  start=list(a=1, b=1, c=0, d=-0.8))

  data$est_afr_upper_approx <- predict(u_model7, data)
  data$est_afr_lower_approx <- predict(l_model7, data)
  ...
  geom_text(
    label=paste("",
                paste("model7", format(formula(u_model7)), collapse=""),
                paste(paste0(paste0(names(coef(u_model7)),"="),
                             signif(coef(u_model7), digits=6)), collapse=" "),
                sep="\n"),
    x=Inf,
    y=Inf,
    hjust=1,
  ...
  filename <- sprintf("-v3-upper-%.2f-z1", c_level * 100)
  ...
  g2 <- ggplot(data,
    ...
    theme(plot.title=element_text(hjust=0.5, size=38),
          ...
          legend.title=element_blank()) +
    geom_line(aes(y=est_afr, color='est_afr'), linewidth=1) + 
    geom_line(aes(y=est_afr_lower, color='est_afr_lower'), linewidth=1) +
    geom_line(aes(y=est_afr_lower_approx, color='eal_approx'), linewidth=1.5) +
    scale_colour_manual("", values=c("est_afr"="black",
                                     "est_afr_lower"="red",
                                     "eal_approx"="purple")) +
    labs(y="annualized failure rate") +
  ...
  # geom_text(label="test2",x=Inf,y=Inf, hjust=1, vjust=1, size=8) +
  geom_text(
    label=paste(paste("model7", format(formula(l_model7)), collapse=""),
                paste(paste0(paste0(names(coef(l_model7)),"="),
                             signif(coef(l_model7), digits=6)), collapse=" "),
                "",
                sep="\n"),
    x=Inf,
    y=-Inf,
  ...
  g2a <- g2 + coord_cartesian(ylim=c(0.0, 1.0)) + facet_zoom(xlim=c(0, 20), zoom.size=1)
  g2b <- g2 + coord_cartesian(ylim=c(0.8, 1.0)) + facet_zoom(xlim=c(900, 1000), zoom.size=1)
  
  filename <- sprintf("-v3-lower-%.2f-z1", c_level * 100)
  ggsave(paste(filebase, filename, ".png", sep=""),
         g2a,
         dpi=100, height=24, width=24, units="in", limitsize = FALSE)
